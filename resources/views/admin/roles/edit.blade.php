<div x-data="{ show: false, role: { id: '', name: '', user_id: '' } }"
     x-show="show"
     @open-modal.window="if ($event.detail.name === 'edit-role') { role = $event.detail.role; show = true; }"
    ...
    <div @click.outside="show = false" class="bg-white dark:bg-neutral-primary-soft rounded-2xl shadow-xl border border-default w-full max-w-md overflow-hidden p-6 font-khmer">
        <div class="flex justify-between items-center pb-4 mb-4 border-b border-default">
            <h3 class="text-lg font-bold text-heading font-english">{{ __('Edit Role') }}</h3>
            <button type="button" @click="show = false" class="text-body hover:text-heading cursor-pointer text-xl font-bold">&times;</button>
        </div>
        ...
        <form :action="'/admin/roles/' + role.id" method="POST" class="space-y-4">
            ...
            <!-- Role Name -->
            <div>
                <label class="block text-xs font-bold text-body uppercase mb-1 font-english">{{ __('Role Name') }}</label>
                <select name="name" x-model="role.name" required 
            ...
            <!-- Assigned User -->
            <div>
                <label class="block text-xs font-bold text-body uppercase mb-1 font-english">{{ __('Select User') }}</label>
                <select name="user_id" x-model="role.user_id" required 
                        class="w-full px-4 py-2.5 bg-neutral-secondary-medium border border-default-medium rounded-xl text-heading text-sm font-english focus:ring-2 focus:ring-primary focus:border-primary">
